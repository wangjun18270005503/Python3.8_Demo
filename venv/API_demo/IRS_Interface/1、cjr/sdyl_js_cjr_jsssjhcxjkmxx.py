...
# @Author : J.wang 
# @Version: V 0.1 
# @File : sdyl_js_cjr_jsssjhcxjkmxx.py
# @Software: PyCharm
# @desc : todo 模拟真实 刷调用量 江山公共数据平台类型接口 残疾人项目 江山市手机号查询健康码信息
'''
实现思路：
 1、脚本启用定时调度，每分钟执行一次。
...
def gateway(url, appKey, appSecret, key_name):
    ...
    # 获取随机 sfzh
    select_sql = "SELECT sjh FROM `js_population_info` WHERE  sjh REGEXP '^[1][3-9][0-9]{9}$' LIMIT " + str(
        random.randint(1, 510000)) + ",1"
    db = create_engine(db_url)
    sfzh_df = pd.read_sql(select_sql, db)
    sfzh_list = sfzh_df['sjh'].to_list()
    if len(sfzh_list) != 0:
        data = {"sjh": sfzh_list[0]}
        print(data)
        # 组合参数
...
def Simulate_reality(url, appKey, appSecret, key_name):
    ...
    if c_time[3:5] == '00':  # 判断截取分钟是否为0
        ...
        print(ran_list)
        # 执行时间点存放至 redis
        r.set(key_name + "_minute_point_wc3r1Dy7cNbYRe70", ran_list)
    now_minute = time.strftime("%M", time.localtime())
    minute_list = json.loads(r.get(key_name + "_minute_point_wc3r1Dy7cNbYRe70"))
    print(minute_list)
    # 判别当前时间与
...
if __name__ == '__main__':
    ...
    url = 'https://10.27.168.100:9443/wc3r1Dy7cNbYRe70/bp?'
