...
# @Time : 2022/10/17 17:18 
# @Author : J.wang 
# @Version: V 0.1 
# @File : sdyl_js_cjr_jsstkryzcryxx.py
# @Software: PyCharm
# @desc : 模拟真实 刷调用量 江山公共数据平台类型接口 残疾人项目 江山市特困人员在册人员信息
...
实现思路：
 ...
 4、在MySQL中准备一张江山市特困人员在册人员信息表（行政区划代码）。
 5、在调用接口时：行政区划代码入参取江山市特困人员在册人员信息表中随机进行提取，可使用分页查询的方式，每页一条，使用随机数（大小依数据量确定0~*）生成随机页。
'''
...
def gateway(url, appKey, appSecret, key_name):
    ...
    # 获取随机 sfzh
    select_sql = "SELECT administrative_division_code FROM `warehouse_dajiuzhu_tekun_hu_detail_code` WHERE administrative_division_code IS NOT NULL AND administrative_division_code != '' GROUP BY administrative_division_code  LIMIT " + str(
...
def Simulate_reality(url, appKey, appSecret, key_name):
    ...
    if c_time[3:5] == '00':  # 判断截取分钟是否为0
        ...
        print(ran_list)
        # 执行时间点存放至 redis
        r.set(key_name + "_minute_point_tekunhuxingzhengquhuachaxun_zhurenwu", ran_list)
    now_minute = time.strftime("%M", time.localtime())
    minute_list = json.loads(r.get(key_name + "_minute_point_tekunhuxingzhengquhuachaxun_zhurenwu"))
    print(minute_list)
    # 判别当前时间与
...
if __name__ == '__main__':
    ...
    url = 'https://10.27.168.100:9443/tekunhuxingzhengquhuachaxun_zhurenwu?'
