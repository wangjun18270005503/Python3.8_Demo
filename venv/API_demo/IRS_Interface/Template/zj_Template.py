...
# @Time : 2022/10/14 9:20
# @Author : J.wang
# @Version: V 0.1
# @File : zj_Template.py
# @Software: PyCharm
# @desc : 浙江省 省级公共数据平台接口 调用模板
'''
实现思路：
 1、脚本启用定时调度，每分钟执行一次。
...
def gateway(url, appKey, appSecret, key_name):
    ...
    if len(sfzh_list) != 0:
        sfzh = sfzh_list[0]
        # 组合参数
        url = url + 'requestTime=' + requestTime + '&appKey=' + appKey + '&sign=' + sign_r + '&sfzh=' + sfzh
        response = requests.post(url)
        print(response.text)
        if response.json()['code'] == '14' or response.json()['code'] == '11':
...
def Simulate_reality(url, appKey, appSecret, key_name):
    ...
    if c_time[3:5] == '00':  # 判断截取分钟是否为0
        ...
        print(ran_list)
        # 执行时间点存放至 redis
        r.set(key_name + "_minute_point_uU4lb0350783d2fa", ran_list)
    now_minute = time.strftime("%M", time.localtime())
    minute_list = json.loads(r.get(key_name + "_minute_point_uU4lb0350783d2fa"))
    print(minute_list)
    # 判别当前时间与
